...
def main():
    ...
    out_path = Path('venv/chromedriver')
    out_path.mkdir(parents=True, exist_ok=True)

    j = requests.get(
        "https://googlechromelabs.github.io/chrome-for-testing/known-good-versions-with-downloads.json").json()
    latest = [v for v in j['versions'] if v['version'].split('.')[:-1] == chrome_version.split('.')[:-1]][-1]

    if platform == "win32":
        d = [e['url'] for e in latest['downloads']['chromedriver'] if e['platform'] == 'win64'][0]
        filename = os.path.split(d)[1]
        r = requests.get(d)
        Path(out_path, filename).write_bytes(r.content)
